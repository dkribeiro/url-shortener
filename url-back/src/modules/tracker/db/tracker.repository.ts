...
export class TrackerRepository {
  ...
  constructor(private dataSource: DataSource) {
    this.counterRepository = this.dataSource.getRepository(TrackerCounterEntity);
  ...
  async incrementCounter(urlId: number): Promise<void> {
    ...
    let counter = await this.counterRepository.findOne({ where: { url_id: urlId } });
  ...
  async logVisit(urlId: number, visitData: {
    user_agent?: string;
    referrer?: string;
    ip?: string;
    location?: string;
  ...
  async trackVisitWithTransaction(urlId: number, visitData: {
    user_agent?: string;
    referrer?: string;
    ip?: string;
    location?: string;
    ...
    await this.dataSource.transaction(async (transactionalEntityManager) => {
      ...
      const counterRepo = transactionalEntityManager.getRepository(TrackerCounterEntity);
      const visitRepo = transactionalEntityManager.getRepository(TrackerVisitEntity);
      ...
      if (!counter) {
        // Create new counter if it doesn't exist
        counter = await counterRepo.save({ urlId, counter: 1 });
      } else {
        // Increment existing counter
      ...
      this.logger.debug(`Successfully tracked visit for URL ID: ${urlId} in a transaction`);
    });
  }

  async getVisitCount(urlId: number): Promise<number> {
    const counter = await this.counterRepository.findOne({ where: { url_id: urlId } });
    return counter ? counter.counter : 0;
  }

  async getVisits(urlId: number, page: number = 1, limit: number = 10): Promise<{ items: TrackerVisitEntity[]; total: number }> {
